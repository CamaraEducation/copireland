...
function wplc_admin_remote_dashboard_thank_you_footer(){
	...
	// echo __("Thank you for using Techspace Chat - Need assistance? Please get in touch here: ", "wplivechat");
...
function wplc_admin_remote_dashboard_scripts($wplc_settings){
	...
	if( get_user_meta( $wplc_current_user, 'wplc_ma_agent', true )) {
        ...
        wp_localize_script( 'wplc-admin-js-agent', 'bleeper_new_chat_notification_text', __("A new chat has been received. Please go the 'Techspace Chat' page to accept the chat", "wplivechat") );
...
function wplc_admin_remote_dashboard_localize_upselling_tips(){
	...
	wp_localize_script( 'wplc-admin-js-agent', 'agent_to_agent_chat_upsell', sprintf(__('Chat to other agents with the <a href="%s" target="_BLANK"></a>.' ,'wplivechat'),'https://wp-livechat.com/purchase-pro/?utm_source=plugin&utm_medium=link&utm_campaign=a2achat'));

	// $initiate_upsell = "<i class='fa fa-horn'></i> ".__("With the Pro add-on of Techspace Chat, you can", "wplivechat");
 ...
 //    $initiate_upsell .= '<strong>'.__("Buy the Pro add-on now.", "wplivechat").'</strong></a>';
	...
	wp_localize_script( 'wplc-admin-js-agent', 'wplc_event_upsell', sprintf( __( 'Get detailed visitor events with the <a target="_BLANK" href="%s"></a>.', 'wplivechat'), 'https://wp-livechat.com/purchase-pro/?utm_source=plugin&utm_medium=link&utm_campaign=pro_events' ) );
         ...
         ?>
            <div class="<?php echo ($is_dashboard ? "" : "update-nag"); ?>" id="wplc_offline_mode_prompt_container" style='margin-top:5px;margin-bottom:5px;<?php echo ($is_dashboard ? "display:none; " : ""); ?>'>
            	...
            	<div style="display: inline-block; margin-left: 80px;">
	                <p style="display: none;"><strong id="wplc_offline_mode_prompt"><?php _e("Techspace Chat - Offline Mode", "wplivechat") ?></strong></p>
...
function wplc_node_compat_pro_update_nag_content(){
	...
	if($is_pro_valid){
		$html_out .= sprintf(__("Chat is now in offline mode, please <a href='%s' target='_BLANK'></a> (or above) of the Techspace Chat Pro Add-on to ensure all functionality works as expected.", "wplivechat"), "update-core.php");
	} else {
	...
	$html_out .= "<p>";
	$html_out .= __("Alternatively, please disable the node server option in the Techspace Chat settings area (Techspace Chat -> Settings -> Advanced Features)", "wplivechat");
	$html_out .= "</p>";
    ...
    if(isset($_GET['page']) && strpos($_GET['page'], 'wplivechat') !== FALSE){
        ...
        if(isset($wplc_settings['wplc_use_node_server']) && $wplc_settings['wplc_use_node_server'] == 1){
            ...
            if($wplc_has_notice_been_dismissed === false){
                ...
                $output .=     "<p><strong>" . __( 'Welcome to V8 of Techspace Chat', 'wplivechat' ) . "</strong></p>";
                ...
                $output .=  "<p>" . __('To activate this functionality please navigate to Techspace Chat -> Settings -> Advanced Features -> And enable our Chat Server option.', 'wplivechat') . "</p>";
