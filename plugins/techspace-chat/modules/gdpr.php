...
function wplc_gdpr_page_layout(){
  ?>
  <h2><?php _e('Techspace Chat - GDPR', 'wplivechat'); ?></h2>
...
function wplc_gdpr_add_wplc_privacy_notice($content){
  ...
  $localized_content = __('We use Techspace Chat as our Techspace Chat platform. By clicking below to submit this form, you acknowledge that the information you provide now and during the chat will be transferred to Techspace Chat for processing in accordance with their %%POLICY_LINK%%.', 'wplivechat');
    ...
    if($_GET['page'] === 'wplivechat-menu-settings'){
      ...
      if(!isset($wplc_settings['wplc_gdpr_enabled']) || $wplc_settings['wplc_gdpr_enabled'] != '1'){
        ...
        if($gdpr_disabled_warning_dismissed === false || $gdpr_disabled_warning_dismissed === 'false'){
          ...
          $privacy_warning = __('Additionally please take a look at Techspace Chat', 'wplivechat');
