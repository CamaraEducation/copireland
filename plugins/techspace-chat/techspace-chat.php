<?php
/*
  Plugin Name: Techspace Chat
  Plugin URI: http://techspace.camara.org
  Description: This is Techspace Chat plugin for COP Project
  Version: 1.0.0
  Author: Tesfaye G.
  Author URI: http://techspace.camara.org
*/
...
function wplc_version_control() {
    ...
    if (!isset($current_version) || $current_version != $wplc_version) {
        ...
        */
	    ...
	    if (!isset($wplc_settings['wplc_pro_fst3'])) { $wplc_settings["wplc_pro_fst3"] = __("Start Techspace Chat", "wplivechat"); }
...
function wplc_admin_menu() {
    ...
    if( get_user_meta( $wplc_current_user, 'wplc_ma_agent', true ) || current_user_can("wplc_ma_agent")){
      $wplc_mainpage = add_menu_page('Techspace Chat', __('Techspace Chat', 'wplivechat'), $cap[0], 'wplivechat-menu', 'wplc_admin_menu_layout', 'dashicons-format-chat');
...
/**
 * Detect if the user is using blocked in the Techspace Chat settings 'blocked IP' section
 * @return void
 * @since  6.0.00
 * @author  Nick Duncan <nick@codecabin.co.za>
 */
 ...
 * @param  array   $wplc_settings Techspace Chat settings array
 * @return string
 * @since  6.0.00
 * @author  Nick Duncan <nick@codecabin.co.za>
 */
	        ...
	        function wplc_desktop_notification() {
	            if (typeof Notification !== 'undefined') {
	                ...
	                var wplc_desktop_notification = new Notification('<?php _e('New chat received', 'wplivechat'); ?>', {
	                    ...
	                    body: "<?php _e("A new chat has been received. Please go the 'Techspace Chat' page to accept the chat", "wplivechat"); ?>"
	                });
	                //Notification.close()
	            }
	        }
	    ...
	    if (function_exists("wplc_register_pro_version")) {
	        ...
	        if (floatval($wplc_pro_version) < 4 || $wplc_pro_version == "4.1.0" || $wplc_pro_version == "4.1.1") {
	            ...
	            <div class='error below-h1'>
	                ...
	                <p><?php _e("You are using an outdated version of <strong>Techspace Chat Pro</strong>. Please", "wplivechat") ?> <a href="https://wp-livechat.com/get-updated-version/" target=\"_BLANK\"><?php _e("update to at least version", "wplivechat") ?> 4.0</a> <?php _e("to ensure all functionality is in working order", "wplivechat") ?>.</p>
	                <p><strong><?php _e("You're Techspace Chat box on your website has been temporarily disabled until the Pro plugin has been updated. This is to ensure a smooth and hassle-free user experience for both yourself and your visitors.", "wplivechat") ?></strong></p>
	        ...
	        if ($wplc_ver < 501) {
	            ...
	            <div class='error below-h1'>
	                ...
	                <p><?php _e("Dear Pro User", "wplivechat") ?><br /></p>
	                <p><?php _e("You are using an outdated version of <strong>Techspace Chat Pro</strong>.", "wplivechat") ?></p>
	                <p>
	                    <strong><?php _e("Please update to the latest version of Techspace Chat Pro", 'wplivechat'); ?>
...
function wplc_first_time_tutorial() {
    if (!get_option('WPLC_FIRST_TIME_TUTORIAL')) {
        ...
        <div id="wplcftt" style='margin-top:30px; margin-bottom:20px; width: 65%; background-color: #FFF; box-shadow: 1px 1px 3px #ccc; display:block; padding:10px; text-align:center; margin-left:auto; margin-right:auto;'>
            ...
            <h2><strong><?php _e("You are now accepting Techspace Chat requests on your site.","wplivechat"); ?></strong></h2>
            <p><?php _e("The Techspace Chat box has automatically been enabled on your website.","wplivechat"); ?></p>
...
function wplc_hook_control_chat_dashboard_bottom() {
 // echo "<p id='wplc_footer_message' style='display:none;'>";
  ?>
  <?php //_e("With the Pro add-on of Techspace Chat, you can", "wplivechat"); ?>
...
function wplc_hook_control_admin_below_chat_box() {
    ...
    echo "  " . __("Assign Quick Response", "wplivechat") . " <select name='wplc_macros_select' class='wplc_macros_select' disabled><option>" . __('Select', 'wplivechat') . "</option></select> <a href='https://wp-livechat.com/purchase-pro/?utm_source=plugin&utm_medium=link&utm_campaign=quick_resposnes' title='" . __('Add Quick Responses to your Techspace Chat', 'wplivechat') . "' target='_BLANK'>" . __("Pro version only", "wplivechat") . "</a>";
    echo "  </div>";
    echo "</div>";
...
function wplc_activate() {
    ...
    if (!get_option("WPLC_SETTINGS")) {
        ...
        $wplc_default_settings_array = array(
            ...
            "wplc_pro_fst3" => __("Start Techspace Chat", "wplivechat"),
...
function wplc_admin_history_layout() {
    wplc_stats("history");
    echo"<div class=\"wrap wplc_wrap\"><div id=\"icon-edit\" class=\"icon32 icon32-posts-post\"><br></div><h2>" . __("Techspace Chat History", "wplivechat") . "</h2>";
...
function wplc_admin_missed_chats() {
    wplc_stats("missed");
    echo "<div class=\"wrap wplc_wrap\"><div id=\"icon-edit\" class=\"icon32 icon32-posts-post\"><br></div><h2>" . __("Techspace Chat Missed Chats", "wplivechat") . "</h2>";
...
function wplc_admin_offline_messages() {
    ...
    echo"<div class=\"wrap wplc_wrap\"><div id=\"icon-edit\" class=\"icon32 icon32-posts-post\"><br></div><h2>" . __("Techspace Chat Offline Messages", "wplivechat") . "</h2>";
    ...
    if(is_admin()){
    	...
    	if (isset( $wplc_settings['wplc_use_node_server'] ) && intval( $wplc_settings['wplc_use_node_server'] ) == 1 ) { } else {
	        ...
	        if(!function_exists('set_time_limit')){
	            $wplc_admin_warnings .= "
	                <p>".__("WPLC: set_time_limit() is not enabled on this server. You may experience issues while using Techspace Chat as a result of this. Please get in contact your host to get this function enabled.", "wplivechat")."</p>
	            ";
	            $wplc_error_count++;
	        }
	        if(ini_get('safe_mode')){
	            $wplc_admin_warnings .= "
	                <p>".__("WPLC: Safe mode is enabled on this server. You may experience issues while using Techspace Chat as a result of this. Please contact your host to get safe mode disabled.", "wplivechat")."</p>
	            ";
	            $wplc_error_count++;
	        }
...
function wplc_support_menu() {
        wplc_stats("support");
?>
    <h1><?php _e("Techspace Chat","wplivechat"); ?></h1>
    <div class="wplc_row">
        ...
        <div class='wplc_row_col' style='background-color:#FFF;'>
            ...
            <hr />
            <p><?php _e("Techspace Chat  has a diverse and wide range of features which may, from time to time, run into conflicts with the thousands of themes and other plugins on the market.", "wplivechat"); ?></p>
...
/**
 * Displays the pointers on teh Techspace Chat dashboard for the initiate chat functionality
...
/**
 * Adds the Techspace Chat & CODECABIN_ footer contents to the relevant pages
...
function wplc_footer_mod( $footer_text ) {
    if (isset($_GET['page']) && ($_GET['page'] == 'wplivechat-menu' ||  $_GET['page'] == 'wplivechat-menu-extensions-page' || $_GET['page'] == 'wplivechat-menu-settings' || $_GET['page'] == 'wplivechat-menu-offline-messages' || $_GET['page'] == 'wplivechat-menu-history')) {
        $footer_text_mod = sprintf( __( 'Thank you for using <a href="%1$s" target="_blank">Techspace Chat</a>! Please <a href="%2$s" target="_blank">rate us</a> on <a href="%2$s" target="_blank">WordPress.org</a>', 'wplivechat' ),
        ...
        return str_replace( '</span>', '', $footer_text ) . ' | ' . $footer_text_mod . ' | ' . __('Techspace Chat is a product of','wplivechat') . ' <a target="_BLANK" href="http://codecabin.co.za/?utm_source=livechat&utm_medium=link&utm_campaign=footer" border="0"><img src="'.plugins_url('/images/codecabin.png', __FILE__).'" style="height:10px;"/></span></a>';
    } else {
        return $footer_text;
    }
...
function wplc_build_api_check($page_content, $data) {
        ...
        if ($data['string'] == "Techspace Chat Pro") {
          $link = "";
          $image = "https://ccplugins.co/api-wplc-extensions/images/add-on0.jpg";
        }
...
function wplc_filter_control_hovercard_bottom_before_pro($content) {
	...
	if(isset($wplc_settings["wplc_powered_by_link"]) && $wplc_settings["wplc_powered_by_link"] == 0){
		...
		$content .= "<a title='".__("Powered By Techspace Chat", "wplivechat")."' target='_blank' rel='nofollow' href='https://wp-livechat.com/?utm_source=poweredby&utm_medium=click&utm_campaign=".esc_html(site_url())."' class='wplc_powered_by_link'>".__("Powered By Techspace Chat", "wplivechat")."</a>";
	}
	return $content;
}
...
function wplc_basic_reporting_page(){
    ...
    $content .= "<h2>".__('Techspace Chat Reporting', 'wp-livechat')." (beta) </h2>";
	...
	$content .= 			"<h3>".__('Techspace Chat Reporting', 'wp-livechat')."</h3>";
...
function wplc_basic_triggers_page(){
	$content = "<div class='wrap wplc_wrap'>";
    $content .= "<h2>".__('Techspace Chat Triggers', 'wp-livechat')." (beta) </h2>";
    $content .= "<script>
    				var isOn = true;
    				jQuery(function(){
    					jQuery(function(){
	...
	$content .= 		"</td>";
	$content .= 		"<td style='vertical-align:top;'>";
	$content .= 			"<h3>".__('Techspace Chat Triggers', 'wp-livechat')."</h3>";
...
function wplc_basic_custom_fields_page(){
	$content = "<div class='wrap wplc_wrap'>";
    $content .= "<h2>".__('Techspace Chat Custom Fields', 'wplivechat')."</h2>";
	$content .= "<table class=\"wp-list-table wplc_list_table widefat fixed form-table\" cellspacing=\"0\" style='width:98%'>";
	$content .= 	"<tr>";
	$content .= 		"<td style='width:35%; vertical-align:top;'>";
	$content .= 			"<img class='trigger_img_main' style='width:99%; height:auto; padding:2px; border:1px lightgray solid;box-shadow: 3px 3px 2px -2px #999;-webkit-box-shadow: 3px 3px 2px -2px #999;-moz-box-shadow: 3px 3px 2px -2px #999;-o-box-shadow: 3px 3px 2px -2px #999;' src='".plugins_url('/images/trigger_sample.jpg', __FILE__)."'>";
	$content .= 		"</td>";
	$content .= 		"<td style='vertical-align:top;'>";
	$content .= 			"<h3>".__('Techspace Chat Custom Fields', 'wp-livechat')."</h3>";
    ...
    <table class='form-table wp-list-table wplc_list_table widefat fixed striped pages' width='700'>
        <tr>
            <td width='350' valign='top'>
                <?php _e("Display a 'Powered by' link in the chat box", "wplivechat") ?>: <i class="fa fa-question-circle wplc_light_grey wplc_settings_tooltip" title="<?php _e("Checking this will display a 'Powered by Techspace Chat' caption at the bottom of your chatbox.", 'wplivechat'); ?>"></i>
            </td>
            <td>
...
function wplc_powered_by_link_in_chat( $string ){
	...
	if( $show_powered_by == 1){
		$ret = "<i style='text-align: center; display: block; padding: 5px 0; font-size: 10px;'><a href='https://wp-livechat.com/?utm_source=poweredby&utm_medium=click&utm_campaign=".esc_html(site_url())."'' target='_BLANK' rel='nofollow'>".__("Powered by Techspace Chat", "wplivechat")."</a></i>";
...
function wplc_basic_version_departments(){
	...
	echo sprintf( "<div class='error'><p>".__("Techspace Chat requires Techspace Chat Pro Version 7.0.0 or greater in order for departments to function as expected. Please update Techspace Chat %s", "wplivechat")."</p></div>", "<a href='".admin_url('update-core.php')."'>".__("here", "wplivechat")."</a>" );
	echo "</div>";
	...
	function wplc_active_chat_box_notices() {
		...
		if ( $wplc_settings["wplc_settings_enabled"] == 2 ) { ?>
            <div class="wplc-chat-box-notification wplc-chat-box-notification--disabled">
                <p><?php echo sprintf( __( 'The Techspace Chat box is currently disabled on your website due to : <a href="%s">General Settings</a>', 'wp-livechat' ), admin_url( 'admin.php?page=wplivechat-menu-settings#tabs-1' ) ) ?></p>
            </div>
			<?php
		} else {
			//$notice = '<div class="wplc-chat-box-notification">';
			//$notice .= '<p>' . __( 'The Techspace Chat box is currently active', 'wp-livechat' ) . '</p>';
			//$notice .= '</div>';
			$notice = '';
...
function wplc_transcript_admin_notice() {
	printf( '<div class="notice notice-info">%1$s</div>', __( 'Please deactivate Techspace Chat Suport - Email Transcript plugin. Since Techspace Chat 8.0.05 there is build in support for Email Transcript.', 'wplivechat' ) );
}
