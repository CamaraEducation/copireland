...
if ( ! class_exists( 'um\admin\core\Admin_Menu' ) ) {
	...
	class Admin_Menu {
		...
		public function admin_footer_text( $footer_text ) {
			...
			if ( isset( $current_screen->id ) && in_array( $current_screen->id, $um_pages ) ) {
				...
				if ( ! get_option( 'um_admin_footer_text_rated' ) ) {
					...
					printf( __( 'If you like Techspace please consider leaving a %s review. It will help us to grow the plugin and make it more popular. Thank you.', 'ultimate-member' ), $link ) ?>
		...
		function primary_admin_menu() {
			$this->pagehook = add_menu_page( __( 'Techspace', 'ultimate-member' ), __( 'Techspace', 'ultimate-member' ), 'manage_options', $this->slug, array( &$this, 'admin_page' ), 'dashicons-admin-users', '42.78578');
			...
			add_submenu_page( $this->slug, __( 'Rating', 'ultimate-member' ), __( 'Rating', 'ultimate-member' ), 'manage_options', $this->slug, array( &$this, 'admin_page' ) );
			add_submenu_page( $this->slug, __( 'Resource Managment', 'ultimate-member' ), __( 'Resource Managment', 'ultimate-member' ), 'manage_options', $this->slug, array( &$this, 'admin_page' ) );
			add_submenu_page( $this->slug, __( 'Chat Managment', 'ultimate-member' ), __( 'Chat Managment', 'ultimate-member' ), 'manage_options', $this->slug, array( &$this, 'admin_page' ) );
		}
		...
		function admin_page() {
			...
			if ( $page == 'ultimatemember' && ! isset( $_REQUEST['um-addon'] ) ) { ?>
				...
				<div id="um-metaboxes-general" class="wrap">
					...
					<h1>Techspace <sup><?php echo ultimatemember_version; ?></sup></h1>
